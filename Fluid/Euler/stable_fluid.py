...
@ti.kernel
def update_vel(vf: ti.template(), pf: ti.template()):
    for i, j in vf:
        pl = sample(pf, i - 1, j)
        pr = sample(pf, i + 1, j)
        pb = sample(pf, i, j - 1)
...
@ti.kernel
def get_2D_pressure(p_1d: ti.types.ndarray(), p_out: ti.template()):
    for I in ti.grouped(p_out):
        p_out[I] = p_1d[I[0] * res + I[1]]
...
def solve_pressure():
    ...
    x = solver.solve(F_b)
    get_2D_pressure(x, pressures_pair.cur)
...
def step(mouse_data):
    ...
    update_vel(velocities_pair.cur, pressures_pair.cur)
