...
class Cloth:
    ...
    def init_pos(self):
        for i, j in ti.ndrange(self.N + 1, self.N + 1):
            ...
            self.vel[k] = ti.Vector([2, 2])
            self.mass[k] = 1.0
            # too small mass will explode? self.mass[k] = 0.1 
    ...
    def display(self, gui, radius=5, color=0xffffff):
        ...
        gui.lines(line_Begin, line_End, radius=2, color=0x0077ff)
...
if __name__ == "__main__":
    ti.init(arch=ti.cpu)
    cloth = Cloth(N=10)
    gui = ti.GUI('Implicit Mass Spring System', res=(600, 600))
    pause = False
    h, max_step = 0.01, 3
    while gui.running:
